...
def display_message(sender: str, message: str):
    ...
    text.append(prefix)
    text.append(
        str(message) if message is not None else "No response."
...
async def chat_loop():  # Changed to async
    ...
    console.print(
        Panel.fit(
            "Welcome to the Health & Wellness AI assistant!", style="bold magenta"
        )
    )
    ...
    while True:
        try:
            ...
            user_input = await asyncio.to_thread(
                input
            )  # Use asyncio.to_thread for input() in async
            ...
            console.print(f"\n[red]Error: {e}[/red]")
            console.print(
                "[yellow]Please ensure your OPENAI_API_KEY is set correctly and you have an internet connection.[/yellow]"
            )
...
def start():  # Typer command is now synchronous
    ...
    try:
        ...
        if "cannot be called from a running event loop" in str(e):
            console.print(
                "[red]Error: Could not start the application due to an existing event loop.[/red]"
            )
            console.print(
                "[yellow]This can happen if the CLI tool is run from an environment that already manages an asyncio loop (e.g., a Jupyter notebook or another async application).[/yellow]"
            )
        else:
